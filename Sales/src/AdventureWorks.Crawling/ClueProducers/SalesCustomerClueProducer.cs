...
namespace CluedIn.Crawling.AdventureWorksSales.ClueProducers
{
    public class SalesCustomerClueProducer : BaseClueProducer<SalesCustomer>
    {
        ...
        protected override Clue MakeClueImpl(SalesCustomer input, Guid id)
        {
            ...
            data.Codes.Add(new EntityCode("/SalesCustomer", AdventureWorksSalesConstants.CodeOrigin, $"{input.CustomerID}"));
            data.Codes.Add(new EntityCode("/SalesCustomer", AdventureWorksSalesConstants.CodeOrigin, $"{input.AccountNumber}"));
