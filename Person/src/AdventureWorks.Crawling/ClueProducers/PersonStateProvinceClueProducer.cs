...
namespace CluedIn.Crawling.AdventureWorksPerson.ClueProducers
{
    public class PersonStateProvinceClueProducer : BaseClueProducer<PersonStateProvince>
    {
        ...
        protected override Clue MakeClueImpl(PersonStateProvince input, Guid id)
        {
            ...
            var clue = _factory.Create(EntityType.Geography.State, $"{input.Rowguid}", id);
            ...
            data.Codes.Add(new EntityCode(EntityType.Geography.State, AdventureWorksPersonConstants.CodeOrigin, $"{input.StateProvinceID}"));
            ...
            {
                _factory.CreateOutgoingEntityReference(clue, "/PersonCountryRegion", EntityEdgeType.PartOf, input.CountryRegionCode, input.CountryRegionCode.ToString());
            }
            if (input.TerritoryID != null && !string.IsNullOrEmpty(input.TerritoryID.ToString()))
            {
                _factory.CreateOutgoingEntityReference(clue, "/SalesSalesTerritory", EntityEdgeType.PartOf, input.TerritoryID, input.TerritoryID.ToString());
            }
