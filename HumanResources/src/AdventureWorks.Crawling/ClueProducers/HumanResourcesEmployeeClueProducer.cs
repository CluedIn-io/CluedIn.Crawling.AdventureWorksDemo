...
namespace CluedIn.Crawling.AdventureWorksHumanResources.ClueProducers
{
    public class HumanResourcesEmployeeClueProducer : BaseClueProducer<HumanResourcesEmployee>
    {
        ...
        protected override Clue MakeClueImpl(HumanResourcesEmployee input, Guid id)
        {
            ...
            var clue = _factory.Create("/HumanResourcesEmployee", $"{input.Rowguid}", id);

            var data = clue.Data.EntityData;

            data.Name = $"Employee {input.BusinessEntityID}";

            data.Codes.Add(new EntityCode("/HumanResourcesEmployee", AdventureWorksHumanResourcesConstants.CodeOrigin, $"{input.BusinessEntityID}"));
            data.Codes.Add(new EntityCode("/HumanResourcesEmployee", AdventureWorksHumanResourcesConstants.CodeOrigin, $"{input.NationalIDNumber}"));
            data.Codes.Add(new EntityCode("/HumanResourcesEmployee", AdventureWorksHumanResourcesConstants.CodeOrigin, $"{input.LoginID}"));
            ...
            {
                _factory.CreateOutgoingEntityReference(clue, EntityType.Person, "Is", input.BusinessEntityID, input.BusinessEntityID.ToString());
            }
