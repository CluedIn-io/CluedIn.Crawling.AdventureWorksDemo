...
{
    public class GetCrawlJobDataBehaviour : AdventureWorksProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(AdventureWorksConstants.KeyName.ConnectionString, nameof(AdventureWorksCrawlJobData.ConnectionString))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task AdventureWorksCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.AdventureWorks.Core.AdventureWorksCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
